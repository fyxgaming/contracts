...
import { createOptimisticSignature } from '../lib/createOptimisticSignature';
import fs from 'fs/promises';
import { BigNumber, ethers } from 'ethers';
import 'dotenv/config';
import { uploadMetadataAndGetCIDsWithFileBase } from '../lib/uploadMetadataAsync';
...
async function main() {
    ...
    signature = resCreateOptimistic?.signature;
    expiry = resCreateOptimistic?.expiry;

    const sigMintInstance =
    ...
    const unsignedTx: any = await sigMintInstance.populateTransaction.mintBatchWithTokenUrisViaSignature(
        destinationAddress,
        tokenUris,
        amounts,
        expiry,
    ...
    console.log('BEFORE SEND TRANSACTION: ', unsignedTx);
